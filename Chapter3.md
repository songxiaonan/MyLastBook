...
两种编程模式几乎诞生于同一年代，但是在随后的几十年中，命令式编程始终占据绝对主流。直到2009年之后，随着多核处理器和并行计算的兴起，由于声明式编程语言（主要是函数式语言）可以大大降低并行程序的编写难度，该模式才开始得到了业界的高度关注。

如果声明指的是针对任务目标的声明，那么所有通用编程语言都是命令式的，原因很简单：计算机无法自动完成所有任务。函数式编程语言虽然也被认为是声明式的，但是也只是过程中的声明，并不是针对任务目标的声明。如果限制在某个具体的领域，实现一个针对任务目标的声明式语言则会相对比较容易：枚举这个领域的所有任务目标，设计一种语言描述这些任务目标，再根据一系列规则得出到达这些任务目标的执行路径。实际上很多流行的领域专用语言（DSL）都是声明式的：因为描述任务目标比描述任务的执行过程要容易的多，所以声明式DSL轻而易举的获得了大量的用户。最常见的声明式DSL就是SQL，用户使用SQL描述想要获取什么样的数据（即声明任务目标），数据库会自行选择获取这些数据的最佳执行路径，并将数据返回给用户。众所周知，使用SQL获取数据，要比使用命令式语言自行编写处理过程去获取数据容易的多：数据库可以帮助用户收集数据的统计信息、维护索引、选择最佳执行计划、并将任务分配到多个CPU甚至多台服务器，这些工作如果由用户自行完成（使用数据库的底层API），代码量可能会有成千上万行。由此可见，如果一个工具软件支持声明式的用户接口，那么它的易用性会得到极大的增强。所以说声明式不仅仅是一个编程泛型概念，还是一个重要的软件设计思想。
...
* 程序员喜欢控制事情的发展，习惯去描述how，不喜欢系统中存在脱离掌控的自动化处理过程
很多时候，我们并不清楚想要的是什么，比如编写业务逻辑的时候，我们只知道业务规则和流程，却对最终结果知之甚少
归纳和提取完备的what并不是一件容易的工作

在微服务模式下，如果微服务的运行平台不能提供声明式接口，那么则会遇到如下问题：

* 需要step-by-step的编写脚本，需要设想目标环境的各种状况、处理各种edge cases，高度依赖编写者的经验
脚本在不同的环境里运行可能会产生不同的结果
编写脚本时需要假定目标环境保持稳定，这在分布式共享环境下不大现实
