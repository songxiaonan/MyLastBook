...
持续交付（Continuous Delivery）旨在更加密集的构建、测试和发布应用，密集到如果业务需要，则每一次修改都可以即时发布到生产环境。这么做的目的是：
...
AKF扩展立方是构建可扩展架构的指导性原则，这里的AKF并不是什么技术缩写，而是Marty Abbott、Tom Keeven和Michael Fisher等三人姓氏的首字母，这三个人创办了一家名为AKF Partners的咨询公司，在如何构建可扩展架构的问题上总结了大量的经验，AKF扩展立方是其中最为著名的一个。

AFK扩展立方把应用架构的扩展方式归类为三个维度，就好像一个立方体的三个轴向，它们分别是：

* X轴代表无差别的克隆服务和数据。
* Y轴代表按任务类型或数据类型（也可以是两者的结合）划分应用架构。
* Z轴代表按请求或用户的来源对应用的部署进行分片。

其中X轴扩展就是我们最常见的集群或者多实例部署模式，这里的“无差别”暗含了前文中12原则的“原则6：以一个或多个无状态的进程运行应用”和“原则8：通过进程模型进行扩展”中的部分内容；Y轴扩展某种意义上就是对应用进行微服务改造，例如开发在线商店时，可以把发货功能和用户管理功能独立为两个微服务，这就是典型的按任务类型和数据类型进行应用划分；如果在对应用进行X轴扩展和Y轴扩展之后，应用的处理能力依然无法满足业务的飞速发展，则需要对应用进行Z轴扩展，典型的方式是将X轴扩展和Y轴扩展之后的应用进行多份部署，每份部署承接来自不同地区的请求。

想必很多读者早已非常清楚这三个维度的扩展方式，AKF扩展立方并不是什么鲜为人知的新发现，它只是一个系统的总结。有意思的是AKF扩展立方并不只适用于应用架构的扩展，它还适用于组织架构的扩展，在后文定律部分的康威定律中，我们可以看到，在对应用架构进行扩展的同时，必须保持组织架构的同步扩展。

本章的开头提到，原则必须能够指导实践，因此即使将场景限制在微服务模式的软件开发之中，通用的原则也不会很多，各位读者需要根据自身的情况设立自己的原则。另外，为了使原则易于遵守，也不宜设立过多的原则，最好可以限制在7到10条以内，本章总共列举了13条原则，想必已经足够，接下来我们将进入定律部分。
...
上个世纪六十年代，著名计算机科学家梅尔文·康威（Melvin Conway）在其职业生涯中观察到了一个有趣的现象：软件产品的架构在很大程度上会反映其开发团队的组织结构。后来康威将其推广为一种社会学观察，并为其撰写了一篇名为“How Do Committees Invent?”的论文，其核心思想被概括为：

>任何设计系统的组织，必然会产生以下设计结果：该系统的结构就是该组织沟通结构的写照。

当时梅尔文·康威非常谦逊的仅仅将这种现象称作是自己的发现和总结，在近十年之后，弗雷德·布鲁克斯（就是定律三里的那个布鲁克斯）在其软件工程著作《人月神华》中才将其奉为“康威定律”，随后又被多部论述软件的著作反复提及。开源运动倡导者埃里克·雷蒙在《新黑客辞典》中提到“软件的组织和软件团队的组织将是一致的”，并特别使用Congruent（意思是全等，就是全等三角形的那个全等）这个单词形象的描述这种一致性。尼尔·哈里森和詹姆斯·科普林在《敏捷软件开发的组织模式》中提到，如果组织的构成不能密切的反映产品的基本部件，或者组织之间的关系不能反映产品部件之间的关系，那么项目将遇到麻烦。简单的说：产品结构是其组织沟通结构的缩影。实际上这里的系统或产品，指的是广义上的系统和产品，并不仅仅是软件，梅尔文·康威最初也是将文章投稿到《哈佛商业评论》的，可惜商界人士并未立即意识到文章的价值，伟大的发现在相当长的时间内仅在程序员圈子内流传。直到半个世纪之后，Google的前任CEO、现任董事长埃里克·施密特（同时也是一位Unix专家）在谈及拙劣的产品设计时曾提到网络电视遥控器的例子：遥控器的点播键又大又醒目，而静音键又小又难找，只是因为点播业务是该企业的主要收入来源，而没人能从静音上赚到钱。这和康威定律所描述的现象十分相似。

在传统的软件开发模式下，组织通常会按技术职能划分为前后端团队、DBA团队和运维团队，根据康威定律所述，这种组织结构显然不适合微服务模式的软件开发。想必本书的读者都已经十分清楚，在微服务模式下，必须按业务功能去划分团队。各位读者也一定听说过杰夫·贝索斯倡导的“Two-pizza Team”，但是问题的核心并不是团队规模的大小，而是“沟通”。在亚马逊公司早期的一次外出静思会（Off-site Retreat）上，有高管提出团队之间应该进行更多的沟通，贝索斯立刻起身发对：“不，沟通很可怕！”这让在场的其他高管非常震惊。在此我们不去探讨贝索斯这个观点在组织管理学上的普遍意义，争议是显而易见的，但是如果你是在管理一个微服务开发团队，贝索斯的观点确切无疑。贝索斯所倡导的是，在缩小团队规模的同时，还必须尽可能的减少团队之间的沟通，尽可能的将沟通限制在小规模团队的内部。让我们回到本小节的开头，不知你是否留意，康威定律所强调的也是组织“沟通结构”对系统结构的影响。在使用微服务模式进行软件开发时，如果不能有效的减少团队之间的沟通，即使采用了小规模团队，也极有可能导致各微服务之间过高的耦合性。一个微服务应用如果在各微服务之间存在过高的耦合性，那就等于在引入了微服务模式的复杂度的同时，又保留了单体应用的主要缺陷，这显然是一种非常糟糕的实践。

在“理念二：协作优先，文本通讯”中我们曾提到使用粗粒度的接口隔离相互协作的程序的内部状态，这在微服务开发中是个非常基本的要求，更进一步的，每个微服务的开发团队都应该为其所负责开发的微服务建立强约束力的服务契约，并尽可能的将与其他团队的沟通限制在服务契约层面，在遵守服务契约的前提下，各团队对微服务的内部实现上拥有充分的自主权，团队之间不应该在微服务的内部实现上做出妥协。从技术和管理两个层面共同保证微服务之间清晰的边界，这是康威定律带给我们的启示，另外，在整个项目周期中，微服务的划分方式很可能会做出多次调整，根据康威定律，这意味着组织结构的相应调整，因此，组织的灵活性对项目的成败也至关重要。

最后需要说的是，康威还是协程的发明者，如果你使用Go语言、熟悉Goroutines或者是其他编程语言的类似机制，康威可以算作是这类机制的鼻祖，得知此事，想必你会对康威钦佩有加，他的发现和发明，分别在宏观层面和微观层面为软件行业带来了深远的影响。